...
<body class="bg-slate-50 text-slate-800 antialiased selection:bg-orange-500 selection:text-white flex flex-col min-h-screen">
    ...
    <script>
        ...
        function openBookingTransactionModal(roomId, roomNumber, period = 'bulanan') {
            ...
            const priceWeekly = Math.round((basePrice * 0.35) / 10000) * 10000;
            const priceMonthly = basePrice;
            const priceSemester = Math.round((basePrice * 6 * 0.95) / 50000) * 50000;
            const priceYearly = basePrice * 11;

            const rentalOptions = {
                'mingguan': { label: 'Mingguan', price: priceWeekly },
                'bulanan': { label: 'Bulanan', price: priceMonthly },
                'semester': { label: 'Semesteran', price: priceSemester },
                'tahunan': { label: 'Tahunan', price: priceYearly }
            };

            const periodInfo = rentalOptions[period] || rentalOptions['bulanan'];
